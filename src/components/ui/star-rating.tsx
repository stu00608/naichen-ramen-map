...
export function StarRating({
  ...
  const getStarValueAtPosition = (starIndex: number, event: React.MouseEvent<HTMLDivElement>) => {
  ...
  const handleStarClick = (starIndex: number, event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (Math.abs(rating - clickedValue) < 0.1) {
      ...
      newRating = step === 0.5 && clickedValue % 1 === 0 
  ...
  return (
    ...
    >
      {Array.from({ length: max }, (_, i) => {
        const starValue = i + 1;
        const isActiveHalf = displayValue >= starValue - 0.5 && displayValue < starValue;
