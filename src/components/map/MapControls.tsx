...
export default function MapControls({ onSearch, onToggleResults, isResultsOpen }: MapControlsProps) {
      ...
      <div className="absolute z-10 top-4 right-4">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="rounded-full p-0 h-10 w-10 bg-background/80 backdrop-blur-sm shadow-md">
              {isAuthenticated ? (
                <Avatar>
              ...
              <>
                <DropdownMenuLabel>
                  <div className="flex flex-col space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground">登入以使用更多功能</p>
                  </div>
                </DropdownMenuLabel>
                <DropdownMenuSeparator />
                <DropdownMenuItem asChild>
                  <Link href="/login" className="w-full">登入</Link>
                </DropdownMenuItem>
                <DropdownMenuItem asChild>
                  <Link href="/signup" className="w-full">註冊</Link>
                </DropdownMenuItem>
              </>
            )}
          </DropdownMenuContent>
