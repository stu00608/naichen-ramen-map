...
import { ArrowLeft, StarIcon, XIcon } from "lucide-react";
...
export default function SearchResults({
	...
	return (
		...
		>
			...
			<div
				className={`${isMobile ? "max-h-[calc(66vh-65px)]" : "h-[calc(100vh-65px)]"} overflow-hidden`}
			>
				{selectedShop ? (
								...
								filteredReviews.map((review) => (
									<Card key={review.id} className="mb-3 bg-card border-border">
										<CardContent className="pt-4">
											...
											<p className="text-sm mt-2 line-clamp-3 text-card-foreground">
												{review.content}
											</p>
										</CardContent>
									</Card>
					...
					>
						<div className="px-4 pt-2">
							<TabsList className="w-full">
								<TabsTrigger value="shops" className="flex-1">
									拉麵店 ({shopResults.length})
								</TabsTrigger>
								<TabsTrigger value="reviews" className="flex-1">
									評論 ({reviewResults.length})
								</TabsTrigger>
							</TabsList>
						</div>
						...
						<TabsContent
							...
							className="p-0 mt-0"
							style={{
								height: isMobile ? "calc(66vh - 110px)" : "calc(100vh - 110px)",
							}}
						>
											...
											>
												<CardContent className="p-3">
													...
													<p className="text-xs text-muted-foreground">
														{review.shopName} · {review.userName} ·{" "}
														{new Date(
															review.createdAt?.toDate(),
														).toLocaleDateString()}
													</p>
