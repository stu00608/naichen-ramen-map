...
const setCookie = (name: string, value: string, days = 14) => {
	...
	const sameSite = window.location.hostname.includes("vercel.app")
		? "None"
		: "Strict";
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
	...
	useEffect(() => {
		const checkStoredAuth = () => {
			...
			const pendingAuthEmailVerified = localStorage.getItem(
				"pendingAuthEmailVerified",
			);
			const pendingAuthTimestamp = localStorage.getItem("pendingAuthTimestamp");

			if (pendingAuthUID && pendingAuthToken) {
				const timestamp = Number.parseInt(pendingAuthTimestamp || "0", 10);
				const now = Date.now();
					...
					if (userDoc.exists()) {
						...
						if (window.location.pathname === "/login") {
							...
							if (authTime && new Date(authTime).getTime() > fiveMinutesAgo) {
								console.log(
									"Recently authenticated but on login page, redirecting to dashboard",
								);
	...
	const signIn = async (email: string, password: string) => {
		try {
			...
			if (methods.includes("google.com")) {
				throw new Error(
					"此帳號已使用 Google 登入。請點擊「使用 Google 登入」按鈕。",
				);
			}
	...
	const signInWithGoogle = async () => {
		try {
			...
			if (userDoc.exists()) {
				...
				console.log(
					"User authenticated but profile not found, redirecting to signup",
				);
			...
			if (err.code === "auth/account-exists-with-different-credential") {
				...
				if (email) {
					const methods = await fetchSignInMethodsForEmail(auth, email);
					setError(
						`此電子郵件已使用${methods.includes("password") ? "密碼" : "其他"}方式註冊。請使用該方式登入。`,
					);
				} else {
